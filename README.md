...
- 弹窗打开期间若在别处编辑同一个笔记，候选块的增删会让序号与打开时**错开一格**；某一条读取为空时会保持当前内容，不会切到空白屏。
...
方向键切换走**公开 API**，不新增核心内部类名依赖：

- 弹窗的数据入口从「一条内容」改为 `TextPopupSource`（`size` / `sourceName` / `sourcePath` / `read(index)`），由 `scanner.ts` 实现，弹窗只按索引惰性读取，不认识 DOM。
- 候选集以「本窗格内**已注入的放大按钮**」为准（而不是重跑一遍标签匹配），因此切换顺序必然等于用户看到的图标顺序，也自动继承标签设置、启用开关与平台判断；空块被排除，不会切到一屏空白。
...
- 候选与内容每次切换都按当前 DOM **重采**，所以弹窗打开期间编辑器重渲染（例如另一窗格在编辑同一笔记）不会读到已脱离文档的节点。
