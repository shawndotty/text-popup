...
- 按住 **`Alt`（macOS 上是 `Option`）点击正文**，会以点击处为锚把内容**放大到 2 倍** —— 像放大镜一样，点击处原地不动；再按住 `Alt` 点一下即**还原**，并回到放大前看到的滚动位置。放大后**按住空格拖拽**可平移画布（不放大时也能用：宽表格、宽图片放不下时同样管用）。这两个手势只作用于正文区，不影响底部控制条，正文里的 `[[链接]]` 也不会因为 `Alt`+点击而跳转；图片同样吃这个放大（底部控制条的「字号 / 缩放」对图片则仍然无效）。控制条的「恢复默认」会把字号、缩放与视图缩放一起复原。
