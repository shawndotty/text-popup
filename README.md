...
- 打开后可用 **`←` / `→` 在同一个笔记的全部可放大区块之间切换**（六类区块混在一起、按文档顺序），不用关掉弹窗再去找下一个图标。规则与 Obsidian 内置的图片放大（lightbox）一致：只用左右两个键；在最后一个按 `→` 环绕回第一个，在第一个按 `←` 环绕到最后一个；**空块**（点了不会弹窗的块）不参与切换。切换后正文会**回到顶部**，而已调整的**字号 / 缩放会保持**，方便连续阅读。
