...
为什么要转：Obsidian 的**编辑器**不解析 HTML 块里的 Markdown，`**粗体**` 在块里会原样显示成星号。`[[笔记]]`、`[文字](链接)`、`![[图片]]`、`$公式$` 与块级 Markdown（`#` 标题、`- ` 列表、`> ` 引用、`|` 表格）有意**不转**：留原文时弹窗里反而能点、能出图、能渲染（编辑器里则仍显示符号）。

外层标签按选区**有没有换行**分流：单行选区用「单行文本包裹标签」（默认 `p`），含换行时用「多行文本包裹标签」（默认 `div`）：
...
| 单行文本包裹标签 | `p` | 选区**只有一行**时，`Popup Selected Text` 用哪个块级标签包裹文本；选项来自「支持的标签」里的块级标签（行内标签生成的块不会有放大图标）。把 `p` 从「支持的标签」里删掉时，它会自动回落到「多行文本包裹标签」 |
