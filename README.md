...
| 命令 | 作用 |
| --- | --- |
| `Popup Selected Text` | 把选中的 Markdown 包成一个块（用设置里的「包裹标签」，默认 `div`），实时预览里立刻出现放大图标 |
...
生成的结构是「正文压在一行、用 `<br>` 表达换行」，原文的一个空行 = 相邻两个 `<br>`：
...
| 包裹标签 | `div` | `Popup Selected Text` 用哪个块级标签包裹文本；选项来自「支持的标签」里的块级标签（行内标签生成的块不会有放大图标）。把它从「支持的标签」里删掉时会自动回落到下一个合法标签 |
...
- **命令的边界**：`Popup Selected Text` 会拒绝缩进 4 空格以上的选区（会被当成代码块）、位于代码块 / 标注 / 数学块内的选区（会被转成字面文本）、以及已经含有点击块的选区（先用 `Unpopup Selected Text` 还原）；这些情况只弹提示、一个字节都不改。`Unpopup Selected Text` 一次只处理一个块，块内手写的 `<b>` `<i>` `<s>` 会归一成 `**` / `*` / `~~`（反向对照表的既定写法）。
